...
public class CustomBlockLog extends BlockLog implements IMetaName, IHasMod {
    ...
    public CustomBlockLog(String name) {
        ...
        setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, EnumHandlers.EnumType.CRYSTAL_NATURE).withProperty(LOG_AXIS,EnumAxis.X));
    ...
    public void registerModels() {
        for (int a = 0; a < EnumHandlers.EnumType.values().length; a++) {
            OceanHeart.proxy.registerVariantRenderer(Item.getItemFromBlock(this), a, "log_" + EnumHandlers.EnumType.values()[a].getName(), "inventory");
        }
    }
