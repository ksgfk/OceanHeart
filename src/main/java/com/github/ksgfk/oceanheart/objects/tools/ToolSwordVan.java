...
public class ToolSwordVan extends ItemAxe implements IHasMod {
    ...
    public boolean hitEntity(ItemStack stack, EntityLivingBase victim, EntityLivingBase player) {
        ...
        player.sendMessage(new TextComponentString(Config.swordVanInfo_2 + rageValue + "/" + rageLimit));
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        if (rageValue == rageLimit) {
            if (rageSwitch == true) {
                ...
                player.sendMessage(new TextComponentString(Config.swordVanInfo_4));
            } else {
                item.getTagCompound().setBoolean("switch", true);
                player.sendMessage(new TextComponentString(Config.swordVanInfo_4));
            }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        ...
        try {
            tooltip.add(Config.swordVanInfo_1);
            tooltip.add(Config.underline);
            tooltip.add(Config.swordVanInfo_2 + rageValue + "/" + rageLimit);
            tooltip.add(Config.swordVanInfo_3);
            tooltip.add(Config.underline);
